...
* Description: This script will perform an out-of-sample estimation to predict the death rate in Medicaid managed-care using a logit prediction model that was trained in Medicare data.
*------------------------------------------------------------------------------*
...
use death_ind_discharge AGE year niss_6_to_5 RTI_RACE_CD SEX_IDENT_CD BENE_RSDNC_SSA_STATE_CD using medicare_from_icdpicr_w_niss_six_to_five.dta, clear
...
label define SEX_label 1 "M" 2 "F" 0 "Unknown"
...
* Run logit
logit death_ind_discharge i.niss_bands i.RACE i.SEX i.STATE c.AGE i.year

*----- Second, read in Medicaid data to predict ---*

* Import Medicaid Data MC
cd "/mnt/labshares/sanghavi-lab/Nadia/data/data_quality_project/medicaid_ffs_mc/data_for_prediction_model/icdpicr/"
use death_ind_discharge AGE year niss_6_to_5 EL_RACE_ETHNCY_CD EL_SEX_CD STATE_CD using mc_from_icdpicr_w_niss_six_to_five.dta, clear
...
gen SEX = .
replace SEX=1 if EL_SEX_CD=="M"
replace SEX=2 if EL_SEX_CD=="F"

* Change values in RACE variable so it's consistent with Medicare (go over with Prachi)
gen RACE = .
replace RACE=1 if EL_RACE_ETHNCY_CD== "1"
replace RACE=2 if EL_RACE_ETHNCY_CD== "2"
replace RACE=3 if EL_RACE_ETHNCY_CD== "8"
replace RACE=4 if EL_RACE_ETHNCY_CD== "4" | EL_RACE_ETHNCY_CD== "6"
replace RACE=5 if EL_RACE_ETHNCY_CD== "5" | EL_RACE_ETHNCY_CD== "7"
replace RACE=6 if EL_RACE_ETHNCY_CD== "3"
...
margins niss_bands, noesample saving(predicted_death_medicaid_mc_logit_model, replace)
